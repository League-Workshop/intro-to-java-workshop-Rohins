...
public class RobotTreasureHunt implements KeyEventDispatcher{
	...
	private void spaceBarWasPressed() {
		...
		if(robot.getX()==720 && robot.getY()==415){
		JOptionPane.showMessageDialog(null, "Go to the parrot!");	
		}
		if(robot.getX()==615 && robot.getY()==145){
			JOptionPane.showMessageDialog(null, "Go to the fish!");	
			}
		if(robot.getX()==475 && robot.getY()==550){
			JOptionPane.showMessageDialog(null, "Go to the boy!");	
			}
		if(robot.getX()==205 && robot.getY()==390){
			JOptionPane.showMessageDialog(null, "Go to the bottle!");	
			}
		if(robot.getX()<=530-640 && robot.getY()<=295-475){
			JOptionPane.showMessageDialog(null, "lol u got clapped by the pirate robot");	
			}
		}
